...
3. Click **Add Environment Variable** and enter:
   ...
   - **Value**: _Your Docker Hub username_ (e.g., `lalithtrs`)
4. Click **Add Environment Variable** again and enter:
   - **Name**: `DOCKERHUB_TOKEN`
   - **Value**: _Your Docker Hub Personal Access Token (PAT)_
...
By default, CircleCI runs the pipeline when _any_ file in the repository changes. In a monorepo (which contains `CCABank`, `mlapp`, and `To-Do-app`), this wastes execution minutes.
